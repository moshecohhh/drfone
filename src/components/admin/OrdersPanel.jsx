...
export default function OrdersPanel({ focusId = null }) {
  const { orders, updateStatus, updateOrderItems, issueInvoice, createPaymentLink, chargeOrder, setOrderDraft, setOrderPayments, getInvoicePdf, deleteOrder, addOrderLog, markOrderRead } = useOrders()
            ...
            {isOpen && (
              <div className="grid gap-4 border-t border-black/5 bg-brand-50/30 p-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  <OrderEditor order={o} updateOrderItems={updateOrderItems} updateStatus={updateStatus} issueInvoice={issueInvoice} createPaymentLink={createPaymentLink} chargeOrder={chargeOrder} setOrderDraft={setOrderDraft} getInvoicePdf={getInvoicePdf} />
                </div>
...
function OrderEditor({ order, updateOrderItems, updateStatus, issueInvoice, createPaymentLink, chargeOrder, setOrderDraft, getInvoicePdf }) {
  ...
  // Z-Credit tokenize-then-charge flow. The customer saves their card (at
  // checkout, or via a link the admin sends here); once a token is saved the
  // admin charges the order's current total. 'paid' is set by the charge.
  const payUrl = order.zcredit?.sessionUrl || ''
  const paid = order.paymentStatus === 'paid'
  const hasToken = !paid && (order.paymentStatus === 'card_saved' || !!order.zcredit?.token)
  ...
  const onCharge = async () => {
    if (!window.confirm(`לחייב את כרטיס הלקוח ב-${money(order.total)}?`)) return
    setPayErr('')
    setPayBusy(true)
    const res = await chargeOrder(order, order.total)
    setPayBusy(false)
    if (!res.ok) { setPayErr(res.error || 'החיוב נכשל'); return }
  }
    ...
    <div className="rounded-xl border border-black/5 bg-white p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* Z-Credit: paid badge → charge saved card → card-save link state. */}
          {paid ? (
            ...
            <button type="button" onClick={onCharge} disabled={payBusy} className="flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-700 disabled:opacity-60">
              <CreditCard size={14} /> {payBusy ? 'מחייב…' : `חיוב כרטיס ${money(order.total)}`}
            </button>
          ) : payUrl ? (
            <>
              <a href={payUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 rounded-lg border border-brand-200 bg-brand-50 px-3 py-1.5 text-xs font-bold text-brand-700 hover:bg-brand-100">
                <CreditCard size={14} /> שמירת כרטיס
              </a>
            ...
            <button type="button" onClick={onPaymentLink} disabled={payBusy} className="flex items-center gap-1.5 rounded-lg border border-black/10 px-3 py-1.5 text-xs font-bold text-ink hover:bg-black/5 disabled:opacity-60">
              <CreditCard size={14} /> {payBusy ? 'יוצר…' : 'קישור שמירת כרטיס'}
            </button>
          )}
